...
func mkSegment[T any](segSize uint32) *segment[T] {
	...
	seg.life.refs.Store(int64(0))
	seg.life.state.Store(uint64(segReset(seg.loadWord())))
	return &seg
}

func (s *segment[T]) loadWord() segWord {
	return segWord(s.life.state.Load())
}

func (s *segment[T]) casWord(old, new segWord) bool {
	return s.life.state.CompareAndSwap(uint64(old), uint64(new))
}
...
func (s *segment[T]) resetForUse() {
	...
	s.life.refs.Store(int64(0))
	s.life.done.Store(int64(0))
	s.life.inPool.Store(false)
